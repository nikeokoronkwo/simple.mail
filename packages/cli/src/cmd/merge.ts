...
export default new Command("merge")
    ...
    .description("The given command performs a mail-merge on a given template given a data file. The generated files are output to a given directory")
    .addArgument(new Argument("<template>", "The template file to perform merge on").argRequired())
    .addOption(
        new Option(
    ...
    .action(async (template, options, command) => {
        const { templateEngine: engine, data: { file: dataFile, format: dataFormat }, output } = options;
